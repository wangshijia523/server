...
namespace 服务端
{
	...
	public class ServerSjmyHelper
	{
		...
		public string Site = @"\\192.168.255.241";

		public string Account = "blacknull";

		public string Password = "ndqatest2021.*+";

		public string ApkSite = @"\\192.168.255.241\upload\";
				...
				try
				{
					...
					if (being.Rows.Count == 0)
					{
						var result = SqlHelper.ExecuteDataSet(
							CommandType.Text,
							"select MTID,GNPY,SID,AppSrc,ScriptType,UID,Uninstall from MobileTaskTB  where (GNPY='sjmy' or GNPY='xsjmy'or GNPY='myht') and ScriptFlog=0 order by MTID",
							new List<SqlParameter>().ToArray()).Tables[0];
		...
		public bool CopyCheck(string ip, string newBase, string oldBase, string account, string password)
		{
			...
			var cmd = $@"wmic /node:'{ip}' /password:'{password}' /user:'{account}' process call create '{text}'";
		...
		public int ResetOne()
		{
			SqlHelper.ExecteNonQuery(
				CommandType.Text,
				$"update MobileScriptModule set flog=null,mobile=null,uploadflog=null,ip=null where PacketChannel= '{Gnpy}'",
		...
		var being = SqlHelper.ExecteNonQuery(
				CommandType.Text,
				$"update MobileScriptModule set flog={Mtid},mobile=null,uploadflog=null,ip=null where uploadflog !=1 and PacketChannel= '{Gnpy}' and SID not in({iniFile.IniReadValue("server", "Sid")})",
				new List<SqlParameter>().ToArray());
		...
		private DataTable GetDataSet()
		{
			return SqlHelper.ExecuteDataSet(
				CommandType.Text,
				$"select CHscript, ENscript, mobile, ip from MobileScriptModule where uploadflog != 1 and  PacketChannel = '{Gnpy}'",
				new List<SqlParameter>().ToArray()).Tables[0];
		}
		...
		public string GetPath()
		{
			...
			{
				path +=
					$"{dataTable.Rows[i][0]}:{dataTable.Rows[i][1]}:{dataTable.Rows[i][3]}@{dataTable.Rows[i][2]}{dataTable.Rows[i][0]}测试总报告,";
			}
		...
		#region 清除交易服数据
		public string DelDeal(int mdid, string Gnpy)
		{
			var filePath = $@"d:\\developer\交易服备份\cq_far_trade{mdid}{Gnpy}.sql";
			var dataBaseName = "sjmy_TradeServer";

			if (Gnpy == "xsjmy")
			{
				dataBaseName = "sjmyxsj_TradeServer";
			}
			var result = MySqlHelper.BackupsTrade("172.24.140.83", "wjq", "wjq", dataBaseName, "cq_far_trade", filePath);
			MySqlHelper.InitConnectStr("172.24.140.83", "wjq", "wjq", dataBaseName);
